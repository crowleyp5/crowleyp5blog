...
From these graphs, we can see that players in the top role have a wide range but fairly symmetric distribution of CS differential. The mid role for the blue side tends to have a slight edge in this category, and the bot role for the red side tends to have a slight edge. In terms of kills, it appears more even for the red and blue sides for the top and mid roles, but the bot role has a very apparent edge on the blue side. This could mean one of two things. Firstly, the blue side could be playing more towards the dragon, which is the objective on the bottom side of the map. It could also mean that the blue side has better 5 on 5 team fight potential, which is where the bot role is able to output most of its damage. Overall, it appears that the blue side generates more gold from CS and kills.
